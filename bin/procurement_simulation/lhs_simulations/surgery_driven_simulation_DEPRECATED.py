...
from scm_simulation.order_policies import DeterministicConDOIPolicyV2
from scm_simulation.run_sim import run_stochastic_surgery_driven_simulation, Stochastic_Surgery_Config
...
results_dir = path.join("random_surgery_results", "{0}_condoi={1}".format(item_id, condoi_level))
...
for surgery in surgeries:
    surgery_demand_dist = SurgeriesPerDayDistribution().get_data(usage_df, filters={"dim": "fill_qty",
                                                                                    "op": "eq",
                                                                                    "val": surgery})
    surgery_stochastic_demand[surgery] = GenerateFromSample(list(surgery_demand_dist))

    item_stochastic_demand_sample = list(usage_df[usage_df["fill_qty"] == surgery]["used_qty"])
    print(item_stochastic_demand_sample)
...
hospital = run_stochastic_surgery_driven_simulation(config, show=True)
for surgery in surgeries:
    title = "Distribution of Surgeries per Day Label={0}".format(str(surgery))
