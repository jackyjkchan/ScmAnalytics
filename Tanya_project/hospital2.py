...
class Hospital2(Hospital):
    ...
    def setrandomvars(self, horizon):
        ...
        self.sum_surgery = {surgery: [(int(self.new_booked_surgery_stochastic_demand[surgery][i]) +
                                       int(self.new_surgery_stochastic_demand[surgery][i]))
                                      for i in range(0, horizon, 1)] for surgery in self.surgeries}
        self.new_surgery_item_usage = {surgery: {item: [sum(self.surgery_item_usage[surgery][item].gen()
                                                            for _ in range(0, int(self.sum_surgery[surgery][i])))
                                                        for i in range(0, horizon, 1)] for item in self.item_ids}
                                       for surgery in self.surgeries}
